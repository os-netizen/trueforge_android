...
The dashboard's left navigation icons are still prototype placeholders without handlers. Everything else — session selection, the transcript, its filters, the four inspector tabs, Stop, New, prompt entry and Send — is implemented.

## Dashboard diagnostics and session continuation

A run is a TrueForge *session*, not a single prompt. `POST /dashboard/runs` accepts an optional `runId`; with it the prompt becomes a new turn on that run's existing session, so the agent keeps what it just observed on the device. Without it a fresh session starts. The NDJSON envelope contract the phone (`TaskRunClient.kt`) and `evals/phone-pipeline.ts` depend on is unchanged; `transcript.item` envelopes were added alongside it.

The raw event stream is not viewable as-is. One assistant turn arrives as a `model.message` plus hundreds of `model.message.delta` fragments, and `tool.response` carries only a `toolCallId` — never a tool name. Worse, the model reaches MCP tools through TrueForge's `call_tool` wrapper, so `function.name` is `call_tool` for every device action and the real target sits in the arguments as `tool_name` / `mcp_server` / `input`. `dashboard/transcript.ts` folds all of that into ordered transcript items (thinking, assistant text, tool call paired with its result and latency, approvals, turn boundaries) and `unwrapToolCall` undoes the wrapper. Anything reading tool names off the wire must unwrap it too.

History does not depend on this process. TrueForge persists already-merged events, so `GET /dashboard/runs/{id}/transcript` rebuilds a finished run through the same builder (`buildTranscriptFromSession`), and `GET /dashboard/runs` merges in-memory runs with `sessions.list({ agentId })`, marking recovered ones `historical`. Page limits differ per endpoint: `listTurns` caps at 25, session and turn events at 100.

Analytics come from TrueForge itself, not from the runtime's console — its UI at 8790 is a configuration surface (Agents, Models, Skills, Settings) with no session viewer or metrics page. `turn.done` carries `state.metrics` (input/output/total/cache-read/cache-write/reasoning tokens, and `totalCostInUsd` when the provider reports it; the opencode-go custom provider does not), and each `model.message` carries `usage.inputTokensBreakdown` split across harness, instructions, skills, tool definitions and messages. `GET /dashboard/analytics` aggregates across runs and `GET /dashboard/agent` exposes the operating policy the model actually ran under.

Other endpoints: `GET /dashboard/runs/{id}/events` returns raw events (live buffer while running, TrueForge's merged history afterwards).
...
## Reasoning effort and captured frames (Brief 05)

Reasoning effort is a runtime setting, not a constant. `GET /api/dashboard/reasoning` reports the current levels and the enum the provider accepts (`none, minimal, low, medium, high, xhigh, max` — anything else is rejected upstream with an opaque 400); `POST` with `{agent}` and/or `{vision}` changes them. Both default to `low` (`REASONING_EFFORT`, `VISION_REASONING_EFFORT` override at boot). The two are separate on purpose: the agent's level lives on its manifest, so a change costs a manifest write and lands on the *next* turn, while the vision level is read per call and lands immediately. `agentManifestUpdated: false` in the response means the value did not move, not that it was ignored. Vision effort is also a truncation risk, not just cost — that model bills reasoning against the same `max_tokens` as its answer.

Screenshots never travel through the transcript. `tool.response.content` is a string the transcript clips at 4,000 characters, so an inlined frame arrives both unusable and huge. `capture_screenshot` and `inspect_screen_visually` store the frame in `apps/server/src/media/frames.ts` and put only `frameId` plus both geometries in their text content; `GET /api/frames/{id}` serves the bytes. The store is in-memory and capped at 60 frames, oldest evicted first — an old run's frame is gone by design (the tool promises the image is never persisted), and the dashboard shows "Frame expired" rather than a broken image. Anything else that captures a frame becomes renderable simply by including `frameId`: `extractFrame` matches the field, not the tool name.
